// src/pages/ApproveLand.tsx
import { useState } from 'react';
import { useContract } from '../hooks/useContract';
import {
  formatTimestamp, isValidParcelId, type LandTitle, TitleStatus,
} from '../lib/contract';
import {
  PageHeader, TxFeedback, ConnectPrompt, RoleGuard,
  StatusBadge, ApprovalDots, InfoRow,
} from '../components/ui';

export default function ApproveLand() {
  const {
    wallet, connectWallet,
    verifyTitle, getApprovalStatus,
    approveLand, tx, resetTx,
  } = useContract();
  ...
  if (!canApprove) return <RoleGuard allowed={false} roleName="Registrar, Surveyor, or Approver"><></></RoleGuard>;
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    try {
      ...
      if (!result) { setTitle('not-found'); return; }
      setTitle(result);
  ...
  async function handleApprove() {
    ...
    } catch { /* tx state handled by hook */ }
  }
  ...
  const roleLabel =
    wallet.roles.isApprover  ? 'Senior Approver'
        ...
        <div className="space-y-4 fade-up">
          ...
          <div className="card p-6">
            <div className="flex items-start justify-between gap-3 mb-5 flex-wrap">
              <h2 className="display text-lg text-slate-100 font-semibold mono">{found.parcelId}</h2>
          ...
          <div className="card p-6">
            ...
            {found.status !== TitleStatus.Pending && (
              ...
              >
                <span>◈</span>
                This parcel is <strong className="text-slate-300 ml-1">
                  {found.status === TitleStatus.Approved ? 'already fully approved'
                  : found.status === TitleStatus.Disputed ? 'under dispute'
                  : 'rejected'}
                </strong> — voting is closed.
              </div>
            )}
            ...
            {found.status === TitleStatus.Pending && !alreadyVoted && (
              <>
                <p className="text-slate-400 text-sm mb-4">
                  ...
                  This action is <strong className="text-slate-200">permanent and cannot be undone</strong>.
                </p>
