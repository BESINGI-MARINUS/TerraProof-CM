...
export default function RegisterLand() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!validate()) return;
    if (!file) {
      setUploadErr("Please attach the scanned title document (PDF).");
      return;
    }
  ...
  return (
    <div className="max-w-xl mx-auto">
      <PageHeader
        icon="⊕"
        title="Register Land"
        subtitle="Submit a new land parcel"
      />
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            value={form.parcelId}
            onChange={(e) =>
              setForm((f) => ({ ...f, parcelId: e.target.value.toUpperCase() }))
            }
            disabled={busy}
            spellCheck={false}
          />
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, ownerAddress: e.target.value }))
            }
            disabled={busy}
            spellCheck={false}
          />
          ...
          <p className="text-slate-600 text-xs mt-1">
            The landowner's Ethereum wallet address — provided by the owner at
            the office
          </p>
        </div>
          ...
          >
            ...
            {file ? (
              ...
              <div>
                ...
                <p className="text-slate-600 text-xs mt-1">
                  The scanned <em>titre foncier</em>
                </p>
              </div>
            )}
          </div>
        ...
        >
          <p className="text-amber-400/70 font-medium mb-2">
          ...
          <p>
            ④ Only then does the title reach{" "}
            <span className="text-emerald-400">Approved</span> status.
          </p>
        </div> */}
